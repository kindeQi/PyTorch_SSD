        ...
        if trn_val == 'trn':
            ...
            for idx in range(4):
                for img in self.dataset_json[idx]['images']:
                    self.id_fname[img['id']] = self.root_path_dict[idx] + img['file_name']
            ...
            self.category_idx = {tmp['name']: tmp['id'] for tmp in self.dataset_json[idx]['categories'] for idx in range(4)}
...
if __name__ == "__main__":
    config = Config('local')
    train_dataset = VOC_dataset(config.voc2007_root, config.voc2012_root, config.voc2007_test_anno, trn_val='test')
