...
def get_prior_box():
    ...
    for k, f in enumerate(feature_map):
        for i, j in product(range(feature_map[k]), repeat=2):
            ...
            h.append(min_size[k])
            w.append(min_size[k])
            h.append((min_size[k] * max_size[k]) ** 0.5)
            w.append((min_size[k] * max_size[k]) ** 0.5)

            for ar in aspect_ratio[k]:
                h.append(min_size[k] * ar ** 0.5)
                w.append(min_size[k] / ar ** 0.5)
                h.append(min_size[k] / ar ** 0.5)
                w.append(min_size[k] * ar ** 0.5)
...
def get_iou(bbox, prior_box):
    ...
    area_a = (
        ((gt[:, 2] - gt[:, 0]) * (gt[:, 3] - gt[:, 1])).unsqueeze(1).expand_as(inter)
    )  # [A,B]
    area_b = (
        ((prior_box_[:, 2] - prior_box_[:, 0]) * (prior_box_[:, 3] - prior_box_[:, 1]))
        .unsqueeze(0)
        .expand_as(inter)
    )  # [A,B]
...
def nms(conf_threshold, iou_threshold, top_k, conf, loc, priors):
    ...
    conf_score, conf_cls, loc_ = (
        conf_score[conf_mask],
        conf_cls[conf_mask],
        loc_[conf_mask],
    )
